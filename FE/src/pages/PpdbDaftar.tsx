...
export default function PpdbDaftar() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <p className="text-center text-sm text-body-text/80 mt-2">{step < 4 ? 'Masukkan informasi yang dapat dihubungi oleh pihak sekolah.' : 'Pastikan seluruh data dan dokumen sudah benar sebelum dikirim.'}</p>
      ...
      {step === 1 && (
        <Card className="p-8 space-y-6">
          <div><p className="font-head font-bold">Data Pribadi</p><hr className="mt-2 border-line-soft" />
          ...
          <div><p className="font-head font-bold">Data Sekolah Asal</p><hr className="mt-2 border-line-soft" />
            <div className="grid sm:grid-cols-2 gap-4 mt-4 text-xs font-bold">
          ...
          <div><p className="font-head font-bold">Pilihan Jurusan</p><hr className="mt-2 border-line-soft" />
            <div className="grid sm:grid-cols-2 gap-3 mt-4">
              {JURUSAN.map((j) => (
                <button key={j.kode} onClick={() => set('jurusan', j.kode)} className={`text-left rounded-2xl p-4 text-xs transition-colors ${f.jurusan === j.kode ? 'bg-navy text-white' : 'border border-line-soft bg-white hover:border-brand'}`}>
                  <p className="font-bold">{j.nama}</p><p className={`mt-1 leading-relaxed ${f.jurusan === j.kode ? 'text-white/60' : 'text-body-text/80'}`}>{j.desc}</p>
                </button>
              ))}
            </div>
          </div>
      ...
      {step === 2 && (
        <Card className="p-8 space-y-6">
          {[['ayah', 'Ayah Kandung', 'Wiraswasta'], ['ibu', 'Ibu Kandung', 'Ibu Rumah Tangga']].map(([key, t, ph]) => (
            <div key={key}><p className="font-head font-bold">Data {t}</p><hr className="mt-2 border-line-soft" />
              <div className="grid sm:grid-cols-2 gap-4 mt-4 text-xs font-bold">
          ...
          {wali && (
            <div><p className="font-head font-bold">Data Wali</p><hr className="mt-2 border-line-soft" />
              <div className="grid sm:grid-cols-2 gap-4 mt-4 text-xs font-bold">
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div><h2 className="text-xl font-extrabold">Upload Berkas Pendaftaran</h2><p className="text-xs text-body-text/80 mt-1">Lengkapi dokumen persyaratan di bawah ini untuk melanjutkan pendaftaran Anda.</p></div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {BERKAS.map((b) => (
              <Card key={b.id} className={`p-4 ${files[b.id] ? 'ring-2 ring-brand' : ''}`}>
                ...
                <p className="text-[11px] text-body-text/80 mt-1">{b.f}</p>
                {files[b.id] ? (
                  ...
                  <label className="mt-2 block bg-gray-100 border-dashed border-[1.5px] border-line-soft rounded-xl text-center py-4 text-xs font-bold cursor-pointer hover:border-brand">
        ...
        <div className="grid gap-4 md:grid-cols-[2fr_1fr]">
          <div className="space-y-4">
            <Card className="p-6 text-sm">
              <div className="flex justify-between items-center"><p className="font-head font-extrabold">Data Calon Siswa</p><button onClick={() => setStep(1)} className="text-brand text-xs font-bold">✎ Edit</button></div><hr className="my-3 border-line-soft" />
              <div className="grid grid-cols-2 gap-2.5 text-xs">
                <p><span className="text-body-text/80">Nama Lengkap</span><br /><b>{f.nama || '-'}</b></p>
                <p><span className="text-body-text/80">NISN</span><br /><b>{f.nisn || '-'}</b></p>
                <p><span className="text-body-text/80">Tempat, Tanggal Lahir</span><br /><b>{f.tempatLahir || '-'}, {f.tglLahir || '-'}</b></p>
                <p><span className="text-body-text/80">Jenis Kelamin</span><br /><b>{f.jk || '-'}</b></p>
                <p><span className="text-body-text/80">SMP Asal</span><br /><b>{f.smp || '-'}</b></p>
                <p><span className="text-body-text/80">WhatsApp</span><br /><b>{f.wa || '-'}</b></p>
              </div>
            </Card>
            <Card className="p-6 text-sm">
              <div className="flex justify-between items-center"><p className="font-head font-extrabold">Data Orang Tua</p><button onClick={() => setStep(2)} className="text-brand text-xs font-bold">✎ Edit</button></div><hr className="my-3 border-line-soft" />
              <div className="grid grid-cols-2 gap-2.5 text-xs">
                <p><span className="text-body-text/80">Nama Ayah</span><br /><b>{f.ayahNama || '-'}</b></p>
                <p><span className="text-body-text/80">Pekerjaan Ayah</span><br /><b>{f.ayahKerja || '-'}</b></p>
                <p><span className="text-body-text/80">Nama Ibu</span><br /><b>{f.ibuNama || '-'}</b></p>
                <p><span className="text-body-text/80">Telepon {wali ? 'Wali' : 'Orang Tua'}</span><br /><b>{wali ? f.waliWa || '-' : f.ayahWa || f.ibuWa || '-'}</b></p>
              </div>
            </Card>
            <label className="flex gap-3 bg-gray-100 rounded-2xl p-4 text-xs cursor-pointer">
            ...
            <div className="flex gap-3">
              <button onClick={kirim} className="flex-1 bg-navy hover:bg-brand-hover text-white rounded-xl py-3.5 text-sm font-bold transition-colors">Kirim Pendaftaran →</button>
              <button onClick={() => setStep(3)} className="border-[1.5px] border-brand text-brand rounded-xl px-6 text-sm font-bold">Kembali</button>
            </div>
          </div>
          <div className="space-y-4">
            <div className="bg-navy text-white rounded-3xl p-6 text-sm">
            ...
            <Card className="p-6 text-sm">
              <p className="font-head font-extrabold">Dokumen Upload</p><hr className="my-3 border-line-soft" />
              {BERKAS.map((b) => (
                <p key={b.id} className="py-2 text-xs border-b border-line-soft last:border-0 flex justify-between"><span className={files[b.id] ? 'text-brand font-bold' : 'text-gray-400'}>{files[b.id] ? '✓' : '○'} {b.t}</span>{files[b.id] && <span className="text-gray-400">👁</span>}</p>
              ))}
            </Card>
          </div>
        </div>
      ...
      {step < 4 && (
        <div className="flex justify-between mt-8">
          <button onClick={() => (step === 1 ? (window.location.hash = '#/ppdb/dashboard') : setStep(step - 1))} className="border-[1.5px] border-brand text-brand rounded-xl px-6 py-3 text-sm font-bold">← Kembali</button>
