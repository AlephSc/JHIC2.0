      ...
      <div className="grid gap-6 md:grid-cols-[2fr_1fr]">
        <div className="space-y-6">
          <div>
            ...
            <p className="text-xs text-muted mt-1">📍 {l.lokasi} • {l.tipe} • {l.jurusan}</p>
            ...
            <div className="flex gap-3 mt-5">
              <a href={mailto} className="inline-flex items-center gap-2 bg-navy-950 hover:bg-navy-900 text-white text-sm font-bold px-6 py-3 rounded-xl">Daftar Lowongan →</a>
          ...
          <Card className="p-6">
            ...
            <p className="text-sm text-muted mt-2 leading-relaxed">{l.deskripsi}</p>
            <p className="font-head font-bold text-sm mt-5">Tugas Utama</p>
            <div className="grid sm:grid-cols-2 gap-2 mt-3">
              {l.tugas.map((t) => (
                <p key={t} className="flex gap-2 text-xs text-navy-text bg-gray-100 rounded-xl p-3"><span className="text-brand font-bold">✓</span>{t}</p>
              ))}
            </div>
            <p className="font-head font-bold text-sm mt-5">Kualifikasi</p>
            <ul className="mt-3 space-y-2">
              {l.kualifikasi.map((k) => (
                <li key={k} className="flex gap-2 text-xs text-navy-text"><span className="text-brand font-bold">▸</span>{k}</li>
              ))}
            </ul>
          </Card>
        </div>
        <aside className="space-y-4">
          <Card className="p-6">
            ...
            <p className="text-xs text-muted mt-2">Kirim lamaran beserta CV dan portofolio ke email resmi rekrutmen perusahaan.</p>
          ...
          <Card className="p-6 bg-blue-soft/50 border-blue-soft">
            ...
            <p className="text-xs text-muted mt-2">Guru BKK siap membantu persiapan lamaran dan simulasi wawancara.</p>
