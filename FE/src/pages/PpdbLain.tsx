...
export function PpdbDashboard() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center text-xs border-b border-line pb-3">
      ...
      <div>
        ...
        <p className="text-sm text-muted mt-1">Kelola proses pendaftaran PPDB Anda dari halaman ini.</p>
      </div>
      <div className="grid gap-5 md:grid-cols-[1fr_2fr]">
        <Card className="p-6">
          ...
          <p className="text-xs text-muted mt-2 leading-relaxed">Lengkapi data pendaftaran dan upload berkas untuk melanjutkan proses PPDB.</p>
          <BtnDark to="/ppdb/daftar"><span className="w-full">Lanjutkan Pendaftaran →</span></BtnDark>
        </Card>
        <Card className="p-6">
          <p className="text-[11px] font-bold uppercase tracking-widest text-muted">Progress Pendaftaran</p>
      ...
      <Card className="p-6">
        ...
        <p className="text-xs text-muted mt-1">Gunakan tiket pendaftaran untuk memantau proses verifikasi dan hasil seleksi.</p>
        <p className="mt-3 bg-gray-100 border-dashed border-[1.5px] border-line rounded-xl px-4 py-3 text-xs flex justify-between items-center"><span className="text-gray-400">ID TIKET</span><b>{outbox[0]?.antreId ?? 'PPDB-2026-001245'}</b></p>
      ...
      <div className="bg-navy-950 text-white rounded-3xl p-5 text-xs flex flex-wrap justify-between gap-2">
...
export function PpdbTiket() {
  return (
    <div className="max-w-2xl mx-auto text-center py-6">
      ...
      <p className="text-sm text-muted mt-2">Terima kasih. Data pendaftaran Anda telah kami terima dan sedang dalam proses.</p>
      <Card className="p-8 mt-6 bg-gray-100 text-left">
        <p className="text-center text-[11px] font-bold uppercase tracking-[0.2em] text-muted">Nomor Pendaftaran</p>
        <p className="text-center text-brand font-extrabold text-2xl tracking-[0.15em] mt-1">PPDB-2026-001245</p><hr className="my-4 border-line" />
        <div className="grid grid-cols-2 gap-3 text-xs">
          <p><span className="text-muted">Nama Lengkap</span><br /><b>Budi Santoso</b></p>
          <p><span className="text-muted">Jurusan</span><br /><b>Teknik Komputer dan Jaringan</b></p>
          <p><span className="text-muted">Tanggal Daftar</span><br /><b>24 Oktober 2025</b></p>
          <p><span className="text-muted">Status</span><br /><span className="bg-brand text-white rounded-full px-2.5 py-1 font-bold">Menunggu Verifikasi</span></p>
        </div>
      </Card>
      <p className="bg-blue-badge text-xs rounded-2xl p-4 mt-4 text-left text-navy-text"><b>ⓘ Simpan nomor pendaftaran untuk memantau status.</b> Anda dapat menggunakan nomor ini untuk login dan melihat perkembangan seleksi.</p>
    ...
    <div className="grid gap-6 md:grid-cols-2">
      <div>
        ...
        <Card className="p-7 mt-4">
          <p className="text-[11px] font-bold uppercase tracking-widest text-muted">Status Saat Ini</p>
          <p className="font-head font-bold text-lg mt-2">⏳ Menunggu Proses Seleksi</p>
          <p className="text-sm text-muted mt-2 leading-relaxed">Dokumen pendaftaran Anda telah berhasil kami terima dan saat ini sedang dalam tahap verifikasi administrasi oleh tim panitia penerimaan siswa baru.</p>
          <button className="bg-navy-950 hover:bg-navy-900 text-white text-xs font-bold rounded-full px-6 py-3 mt-5 transition-colors">⟳ Refresh Status</button>
        </Card>
      </div>
      <div className="space-y-5">
          ...
          <div className="flex justify-between mt-4 text-[10px] text-center">
            {[['Pendaftaran Dikirim', true], ['Dokumen Diterima', true], ['Verifikasi Administrasi', false], ['Proses Seleksi', false], ['Hasil Seleksi', false]].map(([l, done]) => (
              <div key={l as string} className="w-16">
                <span className={`w-9 h-9 mx-auto rounded-full flex items-center justify-center font-bold ${done ? 'bg-brand text-white' : l === 'Verifikasi Administrasi' ? 'bg-white ring-[3px] ring-brand text-brand' : 'bg-white border border-line text-gray-400'}`}>{done ? '✓' : '•'}</span>
                <p className={`mt-1.5 font-bold leading-tight ${l === 'Verifikasi Administrasi' ? 'text-brand' : done ? 'text-navy-text' : 'text-gray-400'}`}>{l as string}</p>
              </div>
            ))}
          </div>
        </Card>
        <div>
          ...
          {[['⏰', 'Pastikan nomor telepon dan email yang Anda daftarkan aktif untuk menerima notifikasi jadwal seleksi.'], ['📚', 'Pelajari materi dasar sesuai Program Keahlian yang Anda pilih sebagai persiapan tes masuk.'], ['📖', 'Cek halaman ini secara berkala atau tunggu email resmi dari panitia PPDB.']].map(([ic, t]) => (
            <Card key={t} className="p-4 mt-2.5 text-xs text-muted flex gap-3 items-start">
...
export function PpdbLulus() {
  return (
    <div className="text-center max-w-3xl mx-auto py-6">
      ...
      <p className="text-sm text-muted mt-3">Pencapaian luar biasa. Selamat bergabung dengan ekosistem profesional SMK Telekomunikasi Darul Ulum.</p>
      <div className="grid sm:grid-cols-2 gap-5 mt-8 text-left">
        <Card className="p-6 text-sm">
          ...
          <div className="text-xs mt-4 space-y-3">
            <p><span className="text-muted">NOMOR REGISTRASI</span><br /><b>REG-2026-8924A</b></p>
            <p><span className="text-muted">NAMA LENGKAP</span><br /><b>Bima Satria</b></p>
            <p><span className="text-muted">PROGRAM KEAHLIAN</span><br /><b className="text-brand">Rekayasa Perangkat Lunak</b> &nbsp; TAHUN AKADEMIK <b>2026/2027</b></p>
          </div>
        </Card>
        <div className="bg-navy-950 text-white rounded-3xl p-6 text-sm text-left">
          ...
          <ul className="text-xs mt-4 space-y-2.5 text-white/90">
            <li className="flex gap-2"><span className="text-sky-400">✓</span> Lengkapi dokumen administrasi yang dipersyaratkan.</li>
            <li className="flex gap-2"><span className="text-sky-400">✓</span> Lakukan pembayaran biaya administrasi awal.</li>
            <li className="flex gap-2"><span className="text-sky-400">📅</span> Batas akhir daftar ulang: <b>15 Agustus 2026</b></li>
          </ul>
        </div>
      </div>
