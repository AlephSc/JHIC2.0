    ...
    <div className="space-y-16">
      ...
      <section className="grid gap-8 md:grid-cols-2 items-center">
        <div>
          ...
          <p className="text-sm text-body-text/80 mt-4 leading-relaxed">Kurikulum berbasis industri, fasilitas modern, dan pendampingan karakter. Siap mencetak lulusan yang kreatif, inovatif, dan siap kerja.</p>
      ...
      <section className="bg-navy text-white rounded-[2.5rem] p-8 grid gap-6 md:grid-cols-[2fr_1fr] items-center">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#7db3e8]">Bingung Pilih Jurusan?</p>
        ...
        <div className="flex gap-3 text-[11px]">
          {['01 Jawab Pertanyaan', '02 Analisis Potensi', '03 Dapatkan Rekomendasi'].map((s) => (
            <span key={s} className="bg-[#1b2c46] rounded-xl px-3 py-2.5 font-bold">{s}</span>
          ))}
        </div>
      </section>
      ...
      <section>
        <div className="flex flex-wrap justify-center gap-2">
          {TABS.map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`px-5 py-2 rounded-full text-xs font-bold transition-colors ${tab === t ? 'bg-navy text-white' : 'border border-line-soft bg-white text-navy hover:border-brand'}`}>
        ...
        <div className="grid gap-6 md:grid-cols-[1fr_2fr] items-start mt-8">
          <div>
            ...
            <p className="text-xs text-body-text/80 mt-2 leading-relaxed">{aktif.desc}</p>
            <div className="mt-4 bg-blue-soft/60 border-l-4 border-brand rounded-r-xl p-4 text-xs text-navy"><b>Fokus:</b> {aktif.desc.split(':')[1] ?? aktif.desc}</div>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            {KOMPETENSI[tab].map((k) => (
              <Card key={k.t} className="p-5">
                ...
                <p className="text-xs text-body-text/80 mt-1.5 leading-relaxed">{k.d}</p>
              </Card>
            ))}
          </div>
        </div>
